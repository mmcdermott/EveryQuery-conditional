...
``prob``) and writes ``<metrics_stem>.by_task.parquet``: one row per task with its AUROC and a 95%
bootstrap interval.  Nothing else — no macro, no cross-task intervals.
...
#: One task: a query sequence (codes and windows) under one fixed set of conditioning answers.
TASK_KEY = [
    QUERIES_COL,
    DURATIONS_COL,
    START_DURATIONS_COL,
...
REQUIRED_COLUMNS = [*TASK_KEY[:-1], ANSWERS_COL, SUBJECT_ID_COL, LABEL_COL, PROB_COL]
