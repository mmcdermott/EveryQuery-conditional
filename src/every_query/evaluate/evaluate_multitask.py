...
#: ``SequenceSpec`` windows exactly.  The grid is dense at this level: every spec covers every subject.
...
#: One task cell: a spec, its designed ``forced_answers`` (defaulted to all-null when absent, so not
#: a *required* column), under one fixed set of conditioning answers (see the module docstring).
