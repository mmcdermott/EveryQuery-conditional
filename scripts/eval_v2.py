...
from every_query.generate_tasks.sample_query_sequences import CTX_ID_COL, POSITION_COL, label_binary_occurrence
from every_query.generate_tasks.sample_tasks import _read_event_shard, sample_contexts
...
def sample_eval_contexts(intermediate: Path, split: str, n_per_shard: int, seed: int, limit_shards=None) -> dict:
    """Sample (subject, prediction_time) contexts and return per-shard events for labeling."""
    shard_dir = intermediate / "data" / split
    ctxs, events_by_shard = [], {}
    shards = sorted(shard_dir.glob("*.parquet"))
    if limit_shards:
        shards = shards[:limit_shards]
    for si, fp in enumerate(shards):
        ev = _read_event_shard(fp)
        c = sample_contexts(ev, n=n_per_shard, min_context_per_subject=10, seed=seed + si)
        if c.height:
            c = c.with_columns(pl.lit(si).alias("_shard"))
            ctxs.append(c)
            events_by_shard[si] = ev
...
def main():
    ...
    ap.add_argument("--n-per-shard", type=int, default=512)
    ...
    ce = sample_eval_contexts(args.intermediate, args.split, args.n_per_shard, seed=7,
